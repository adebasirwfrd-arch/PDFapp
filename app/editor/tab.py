...
from PySide6.QtWidgets import (
    ...
    QMessageBox, QDialog, QApplication, QSlider, QCheckBox,
)
import qtawesome as qta

from app.constants import ACCENT, TEXT_PRI, TEXT_SEC, DESKTOP, _LQ, _LP
from app.editor.text_style import (
    insert_style_from_neighbour, insert_multiline_text, neighbour_span_for_insert,
    redact_fill_for_edit, sample_background_fitz, span_color_pdf,
    span_fontname_pdf, span_font_size,
)
from app.editor.area_move import apply_area_move, cleanup_area_move, create_area_move_edit
...
class TabEditar(QWidget):
    ...
    _MODE_KEYS = [
        ...
        ("edit.mode.area_move", "fa5s.object-group"),
    ]
    ...
    def __init__(self, status_fn):
        ...
        self._canvas.overlay_moved.connect(self._on_overlay_moved)
        ...
        self._canvas.set_scroll_area(canvas_scroll)
        ...
        cols = 5  # 11 buttons in a 5×3 grid (last row has 1)
        ...
        # 9 - Move area (select region and reposition)
        w9 = QWidget(); v9 = QVBoxLayout(w9); v9.setContentsMargins(0, 4, 0, 0); v9.setSpacing(4)
        hint9 = QLabel(t("edit.hint.area_move"))
        hint9.setStyleSheet(f"color:{TEXT_SEC}; font-size:11px;")
        hint9.setWordWrap(True)
        self._hint_labels.append(hint9)
        v9.addWidget(hint9)
        v9.addStretch()
        self._opt_stack.addWidget(w9)
        ...
        snap_row = QHBoxLayout(); snap_row.setSpacing(6)
        self._grid_snap_cb = QCheckBox(t("edit.snap.grid"))
        self._grid_snap_cb.toggled.connect(self._on_grid_snap_changed)
        snap_row.addWidget(self._grid_snap_cb)
        snap_row.addWidget(QLabel(t("edit.snap.grid_size")))
        from PySide6.QtWidgets import QSpinBox as _SnapSpin
        self._grid_size_spin = _SnapSpin()
        self._grid_size_spin.setRange(2, 50)
        self._grid_size_spin.setValue(5)
        self._grid_size_spin.setSuffix(" pt")
        self._grid_size_spin.valueChanged.connect(self._on_grid_snap_changed)
        snap_row.addWidget(self._grid_size_spin)
        snap_row.addStretch()
        gpe.addLayout(snap_row)
    ...
    def _on_mode_btn(self, btn):
        ...
        self._canvas.cancel_overlay_drag()
        self._canvas.set_select_mode(idx == 8)
        self._canvas.set_area_move_mode(idx == _MODE_AREA_MOVE)
        is_draw = (idx == 7)
        ...
        self._canvas.set_move_text_enabled(idx in (1, 8))
    ...
    def _on_rect(self, page_idx, pdf_rect):
        ...
        elif mode == _MODE_AREA_MOVE:
            doc = self._canvas._doc
            if not doc:
                return
            edit = create_area_move_edit(doc[page_idx], page_idx, pdf_rect)
            self._add(edit)
    ...
    def _on_point(self, page_idx, pdf_pt):
        ...
        if mode == 1:
            ...
            if not doc:
                return
            page = doc[page_idx]
            near, _line = neighbour_span_for_insert(page, pdf_pt)
            style = insert_style_from_neighbour(
                page, pdf_pt, fallback_size=float(self._text_size.value()))
            if near:
                self._text_size.setValue(
                    max(4, min(144, int(round(style["size"])))))
                self._text_color.set_color(style["color_pdf"])
            self._canvas.begin_inline_text_insert(
                page_idx, style["point"], style["size"],
                style["color_pdf"], style["font"], style.get("flags", 0))
        elif mode == 4:
    ...
    def _on_grid_snap_changed(self, *_args):
        self._canvas.set_grid_snap(
            self._grid_snap_cb.isChecked(),
            float(self._grid_size_spin.value()),
        )

    def _on_text_edit_committed(self, page_idx, edit):
        self._add(edit)

    def _on_overlay_moved(self, idx: int, after: dict, before: dict):
        """Sync pending list after drag-and-drop; record undo if position changed."""
        if not (0 <= idx < len(self._pending)):
            return
        self._pending[idx] = after
        self._canvas.set_overlays(self._pending)
        if overlay_positions_differ(before, after):
            self._redo_history.clear()
            self._redo_stack.clear()
            self._undo_history.append({
                "type": "move",
                "idx": idx,
                "before": clone_overlay_edit(before),
            })
            self._status(t("edit.status.moved"))
        self._canvas.update()
    ...
    def _add(self, edit: dict, *, _from_redo: bool = False):
        ...
        if not _from_redo:
            self._redo_history.clear()
            self._redo_stack.clear()
        self._undo_history.append({"type": "add"})
            ...
            for old in to_drop:
                with contextlib.suppress(Exception):
                    ...
                    if (p and os.path.isfile(p)
                        ...
                        os.unlink(p)
                if old.get("type") == "area_move":
                    cleanup_area_move(old)
        ...
        labels = {
            ...
            "area_move": lambda e: t("edit.label.area_move") + suffix,
        }
    ...
    def _undo(self):
        ...
        if not self._undo_history:
            return
        rec = self._undo_history.pop()
        if rec["type"] == "move":
            idx = rec["idx"]
            before = rec["before"]
            if idx < len(self._pending):
                after = clone_overlay_edit(self._pending[idx])
                self._pending[idx] = clone_overlay_edit(before)
                self._redo_history.append({"type": "move", "idx": idx, "after": after})
                self._canvas.set_overlays(self._pending)
                self._status(t("edit.status.move_undo"))
            return
        if not self._pending:
            return
        edit = self._pending.pop()
        if edit.get("type") == "area_move":
            cleanup_area_move(edit)
        self._redo_history.append({"type": "add", "edit": edit})
        self._redo_stack.append(edit)
        if len(self._redo_stack) > self._MAX_REDO:
            self._redo_stack.pop(0)
        if len(self._redo_history) > self._MAX_REDO:
    ...
    def _redo(self):
        if not self._redo_history:
            return
        rec = self._redo_history.pop()
        if rec["type"] == "move":
            idx = rec["idx"]
            after = rec["after"]
            if idx < len(self._pending):
                before = clone_overlay_edit(self._pending[idx])
                self._pending[idx] = clone_overlay_edit(after)
                self._undo_history.append({"type": "move", "idx": idx, "before": before})
                self._canvas.set_overlays(self._pending)
                self._status(t("edit.status.move_redo"))
            return
        if rec["type"] == "add":
            self._add(rec["edit"], _from_redo=True)
        ...
        * the overlay was an *existing* annotation already present in the
        ...
        for i, p in enumerate(self._pending):
            if p.get("type") == "note" and p.get("text", "").strip() == text and p.get("page") == page:
                ...
                self._redo_history.append({"type": "add", "edit": removed})
        ...
        try:
            ...
            for e in self._pending:
                ...
                if e["type"] == "redact":
                    ...
                    fontname = span_fontname_pdf(e)
                    insert_multiline_text(
                        pg, e["point"], e["text"],
                        fontsize=float(e["size"]),
                        fontname=fontname,
                        color=span_color_pdf(e),
                    )
                    ...
                    if "bg_fill" not in e:
                        fill = sample_background_fitz(pg, bbox)
                    pg.add_redact_annot(bbox, fill=fill)
                    pg.apply_redactions()
                ...
                elif e["type"] == "area_move":
                    apply_area_move(pg, e)
